...
void setEnemyWeight(int cnt)
{
	...
	for (int j = 0; j < cnt; j++)
	{
			...
			{	//showBoard의 1을 0으로 바꿔야함. showBoard에서는 x,y순이므로..
...
void myturn(int cnt) {
	...
	int x[2], y[2];
	FILE *file_pointer;
	file_pointer = fopen("C:\\Users\\최대준\\Desktop\\삼성전자DS부문육목SW알고리즘대회\\01_SamsungGo_AI_Code_예제_공유\\SamsungGo_AI_Code_C_C++\\Releastestlog.txt", "w");
