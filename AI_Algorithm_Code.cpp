...
void myturn(int cnt) {
	...
	// 입출력으로 log 확인
	FILE *file_pointer;
	file_pointer = fopen("C:\\Users\\노유찬\\Desktop\\육목\\삼성전자DS부문육목SW알고리즘대회\\Log\\log.txt", "w");
...
void setMyWeight(int cnt) {
	...
	for (int i = 0; i < cnt; i++) {
		...
		searchMyTree(x, y, x, y, 1);	// level 1부터 시작
	}
}
...
void setEnemyWeight(int cnt) {
	...
	for (int i = 0; i < cnt; i++) {
		...
		searchEnemyTree(x, y, x, y, 1);	// level 1부터 시작
	}
}
	...
	for (int j = y - 1; j <= y + 1; j++) {
		for (int i = x - 1; i <= x + 1; i++) {
			...
			if (pos == 0) {
				...
				val[j][i] = -1;
				if (line == true) searchEnemyTree(x, y, i, j, level + 1);
				else searchEnemyTree(x, y, i, j, 1);
			}
			else {
				if (line == true) {
					val[j][i] -= 20;
